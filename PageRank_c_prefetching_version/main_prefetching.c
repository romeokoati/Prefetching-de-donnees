...
void coo_spmv_build_prefecht(int N, int const * i, int const * j, float const * a, float const * x, float * y) 
{
  #pragma omp parallel
  {
    LIKWID_MARKER_START("pageRank");
    #pragma omp for schedule(static)
      __builtin_prefetch (&j[0]);
      __builtin_prefetch (&x[j[0]]);
      __builtin_prefetch (&a[0]);
      __builtin_prefetch (&i[0]);
      __builtin_prefetch (&y[i[0]]);
    for ( int k = 0; k < N-8; k+=8) 
    {
      __builtin_prefetch (&j[k+8]);
      __builtin_prefetch (&x[j[k+8]]);
      __builtin_prefetch (&a[k+8]);
      __builtin_prefetch (&i[k+8]);
      __builtin_prefetch (&y[i[k+8]]);
      #pragma omp atomic
      y[i[k]] += a[k] * x[j[k]];
      y[i[k+1]] += a[k+1] * x[j[k+1]];
      y[i[k+2]] += a[k+2] * x[j[k+2]];
      y[i[k+3]] += a[k+3] * x[j[k+3]];
      y[i[k+4]] += a[k+4] * x[j[k+4]];
      y[i[k+5]] += a[k+5] * x[j[k+5]];
      y[i[k+6]] += a[k+6] * x[j[k+6]];
      y[i[k+7]] += a[k+7] * x[j[k+7]];
    }
    LIKWID_MARKER_STOP("pageRank");
  }
}
...
void coo_spmv_helper_thread(int N, int const * i, int const * j, float const * a, float const * x, float * y) 
{
  #pragma omp parallel
  {
    LIKWID_MARKER_START("pageRank");
